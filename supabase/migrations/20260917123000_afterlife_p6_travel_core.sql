...
for select to authenticated using (
  exists (
    ...
    where cm.campaign_id = campaign_travels.campaign_id
...
begin
  ...
  ) then raise exception 'NOT_CAMPAIGN_MEMBER'; end if;
  return query
    select * from public.campaign_travels
    where campaign_id = p_campaign_id
    order by case when status in ('planned','in_transit') then 0 else 1 end, created_at desc
...
begin
  ...
  if not exists (
    select 1 from public.campaign_members
    where campaign_id = p_campaign_id and user_id = (select auth.uid()) and role = 'master'
  ) then raise exception 'MASTER_REQUIRED'; end if;
  if p_destination_latitude not between -90 and 90 or p_origin_latitude not between -90 and 90
     or p_destination_longitude not between -180 and 180 or p_origin_longitude not between -180 and 180 then
  ...
  if exists (select 1 from public.campaign_travels where campaign_id = p_campaign_id and status in ('planned','in_transit')) then
    raise exception 'TRAVEL_ALREADY_ACTIVE';
  end if;
  if exists (
    select 1 from unnest(coalesce(p_participant_user_ids,'{}'::uuid[])) uid
    where not exists (
      select 1 from public.campaign_members cm where cm.campaign_id = p_campaign_id and cm.user_id = uid
    )
  ...
  insert into public.campaign_travels(
    campaign_id, created_by, origin_latitude, origin_longitude, destination_latitude, destination_longitude,
    destination_location_id, destination_name, route, distance_m, speed_kmh, estimated_minutes,
    fuel_per_km, fuel_cost, energy_per_km, energy_cost, exposure_per_km, exposure, participant_user_ids
  ) values (
    p_campaign_id, (select auth.uid()), p_origin_latitude, p_origin_longitude, p_destination_latitude, p_destination_longitude,
    p_destination_location_id, left(coalesce(nullif(trim(p_destination_name),''),'Destino no mapa'),140), coalesce(p_route,'{}'::jsonb),
    greatest(0,coalesce(p_distance_m,0)), p_speed_kmh, v_minutes,
    greatest(0,coalesce(p_fuel_per_km,0)), v_fuel, greatest(0,coalesce(p_energy_per_km,0)), v_energy,
    greatest(0,coalesce(p_exposure_per_km,0)), v_exposure, coalesce(p_participant_user_ids,'{}'::uuid[])
...
begin
  ...
  update public.campaign_travels set status='completed', completed_at=now(), updated_at=now()
