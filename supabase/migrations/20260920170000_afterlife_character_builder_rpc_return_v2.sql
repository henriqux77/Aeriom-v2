-- AFTERLIFE character builder: normalize RPC return shapes for PostgREST clients.
drop function if exists public.save_afterlife_character_draft(uuid,jsonb);
...
begin
  ...
  v_age := case when trim(coalesce(p_data->>'age','')) ~ '^\d{1,3}$' then (p_data->>'age')::integer else null end;
  v_height_cm := case
    when regexp_replace(coalesce(p_data->>'height',''),'[^0-9,.]','','g') ~ '^[0-9]+([,.][0-9]+)?$'
    then round(replace(regexp_replace(p_data->>'height','[^0-9,.]','','g'),',','.')::numeric * 100)::integer else null end;

  select * into ch from public.characters where id=p_character_id for update;
  ...
  if ch.id is null then
    ...
    values(
      p_character_id,uid,'draft',v_name,v_age,nullif(trim(p_data->>'gender'),''),
      'Humano',v_origin,v_class,10,10,10,9,0,0,0,
      coalesce(p_data->'attributes','{}'::jsonb),'[]'::jsonb,'[]'::jsonb,'{}'::jsonb,
      coalesce(p_data,'{}'::jsonb),nullif(trim(p_data->>'appearance'),''),
    ...
    update public.characters set
      name=coalesce(v_name,name),age=coalesce(v_age,age),
      gender=coalesce(nullif(trim(p_data->>'gender'),''),gender),
      race='Humano',origin=coalesce(v_origin,origin),class=coalesce(v_class,class),
      ...
      height_cm=coalesce(v_height_cm,height_cm),updated_at=now()
    where id=p_character_id and user_id=uid returning * into ch;
  end if;
  return jsonb_build_object('id',ch.id,'status',ch.status,'user_id',ch.user_id,'creation_state',ch.creation_state);
end
$$;
...
drop function if exists public.finalize_afterlife_character(uuid,jsonb);
...
begin
  ...
  select * into ch from public.characters where id=p_character_id and user_id=uid for update;
  if ch.id is null then
    insert into public.characters(
      id,user_id,status,name,age,gender,race,origin,class,hp_current,hp_max,defense,movement,
      initiative,mana_current,mana_max,attributes,conditions,inventory,equipment,creation_state,
      description,appearance,goals,fears,personality,height_cm,updated_at
    )
    values(
      p_character_id,uid,'completed',v_name,v_age,nullif(trim(p_data->>'gender'),''),
      'Humano',nullif(trim(coalesce(p_data->>'origin_name',p_data->>'origin')),''),
      nullif(trim(p_data->>'class'),''),10,10,10,9,0,0,0,coalesce(p_data->'attributes','{}'::jsonb),
      '[]'::jsonb,'[]'::jsonb,'{}'::jsonb,coalesce(p_data,'{}'::jsonb),
      nullif(trim(p_data->>'appearance'),''),coalesce(p_data->'appearance','{}'::jsonb),
      case when nullif(trim(p_data->>'objective'),'') is null then '{}'::jsonb else to_jsonb(p_data->>'objective') end,
      case when nullif(trim(p_data->>'fear'),'') is null then '{}'::jsonb else to_jsonb(p_data->>'fear') end,
      case when nullif(trim(p_data->>'personality'),'') is null then '{}'::jsonb else to_jsonb(p_data->>'personality') end,
      v_height_cm,now()
    ...
    update public.characters set
      status='completed',name=v_name,age=v_age,gender=nullif(trim(p_data->>'gender'),''),
      race='Humano',origin=nullif(trim(coalesce(p_data->>'origin_name',p_data->>'origin')),''),
      class=nullif(trim(p_data->>'class'),''),attributes=coalesce(p_data->'attributes','{}'::jsonb),
      conditions='[]'::jsonb,creation_state=coalesce(p_data,'{}'::jsonb),
  ...
  insert into public.character_survival(character_id) values(ch.id) on conflict(character_id) do nothing;
  return jsonb_build_object('id',ch.id,'status',ch.status,'user_id',ch.user_id,'creation_state',ch.creation_state,'survival_initialized',true);
end
$$;
