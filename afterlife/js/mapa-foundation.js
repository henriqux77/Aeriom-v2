...
const PLAYER_MAP_ZOOM = 17;
const PLAYER_FREE_PAN_METERS = 460;
const POI_CACHE_TTL = 5 * 60 * 1000;
const POI_QUERY = `[out:json][timeout:8];(nwr(around:900,{LAT},{LNG})[name][shop~"supermarket|convenience|bakery|butcher|hardware|electronics|car_repair|bicycle|sports|books|pet|mall|department_store|variety_store|garden_centre"];nwr(around:900,{LAT},{LNG})[name][amenity~"bar|biergarten|cafe|restaurant|fast_food|food_court|pub|pharmacy|hospital|clinic|doctors|dentist|veterinary|fuel|charging_station|bank|atm|police|fire_station|school|kindergarten|college|university|library|place_of_worship|community_centre|social_centre|shelter|marketplace|cinema|theatre|arts_centre"];nwr(around:900,{LAT},{LNG})[name][leisure~"park|garden|playground|sports_centre|stadium|swimming_pool|fitness_centre"];nwr(around:900,{LAT},{LNG})[name][tourism~"hotel|hostel|museum|attraction|viewpoint|camp_site"];nwr(around:900,{LAT},{LNG})[name][craft];nwr(around:900,{LAT},{LNG})[name][natural~"forest|wood|beach|peak"];);out center tags qt;`;
...
function initMap(){
  map=L.map('worldMap',{
    center:[Number(campaign.latitude)||0,Number(campaign.longitude)||0],
    zoom:role==='player'?PLAYER_MAP_ZOOM:14,
    minZoom:role==='player'?16:2,
    maxZoom:19,
    worldCopyJump:role!=='player',
    zoomControl:true,
    maxBoundsViscosity:role==='player'?.82:0
...
function schedulePois(center){if(role==='player'&&getSelfPosition()&&!center)return;clearTimeout(poiTimer);poiTimer=setTimeout(()=>loadPois(center,map.getZoom()),320);}
...
function renderPoiSet(unique){
  clearPois();
  unique.forEach(p=>{
    const marker=L.marker([p.lat,p.lng],{
      icon:L.divIcon({
        className:'afterlife-poi-icon-wrap',
        html:`<div class="afterlife-poi-icon" aria-hidden="true">${esc(p.icon||'📍')}</div>`,
        iconSize:[30,30],
        iconAnchor:[15,15],
        popupAnchor:[0,-14]
      }),
    ...
    marker.bindPopup(`<strong>\\u200B ${esc(p.name)}</strong><br><span>${esc(p.type)}</span>`);
    marker.bindTooltip(`${esc(p.name)} · ${esc(p.type)}`,{direction:'top',offset:[0,-12],opacity:.96});
    marker.on('click',()=>{
      setTimeout(()=>{
        if(typeof window.__afterlifeOpenLocationFicha==='function'){
          window.__afterlifeOpenLocationFicha({name:p.name,category:p.type,icon:p.icon||'📍',lat:p.lat,lng:p.lng,externalId:p.id?'osm:'+p.id:''}).catch(error=>console.warn('[AFTERLIFE][MAP][FICHA]',error));
        }
      },80);
    });
    marker.addTo(map);
  ...
  if(list){
    ...
    unique.slice(0,18).forEach(p=>{
      const b=document.createElement('button');
      b.type='button';
      b.className='map-poi-row';
      b.innerHTML=`<span class="map-poi-row__icon">${esc(p.icon||'📍')}</span><span><strong>${esc(p.name)}</strong><small>${esc(p.type)}</small></span>`;
      b.addEventListener('click',()=>{
        map.setView([p.lat,p.lng],Math.max(map.getZoom(),16),{animate:true});
        setTimeout(()=>{
          if(typeof window.__afterlifeOpenLocationFicha==='function'){
            window.__afterlifeOpenLocationFicha({name:p.name,category:p.type,icon:p.icon||'📍',lat:p.lat,lng:p.lng,externalId:p.id?'osm:'+p.id:''}).catch(error=>console.warn('[AFTERLIFE][MAP][LIST-FICHA]',error));
          }
        },220);
      });
      list.appendChild(b);
...
async function loadPois(center,zoom){if(zoom<14)return;cleanupPoiCache();if(role==='player'){const self=getSelfPosition();if(!self)return;center=L.latLng(self.lat,self.lng);}const moved=!lastPoiCenter||map.distance(center,lastPoiCenter)>320;const zoomChanged=Math.round(zoom)!==lastPoiZoom;if(!moved&&!zoomChanged)return;lastPoiCenter=L.latLng(center.lat,center.lng);lastPoiZoom=Math.round(zoom);const key=`${center.lat.toFixed(3)},${center.lng.toFixed(3)},${Math.round(zoom)},${role}`;if(key===lastPoiKey)return;lastPoiKey=key;const cached=poiCache.get(key);if(cached){renderPoiSet(cached.data);const status=$('poiStatus');if(status)status.textContent=role==='master'?`${cached.data.length} locais próximos`:`${cached.data.length} locais dentro da sua visão`;return;}const status=$('poiStatus');status&&(status.textContent='Carregando locais…');const q=POI_QUERY.replaceAll('{LAT}',center.lat.toFixed(6)).replaceAll('{LNG}',center.lng.toFixed(6));for(const endpoint of POI_ENDPOINTS){try{const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'text/plain;charset=UTF-8'},body:q});if(!r.ok)throw new Error(`HTTP ${r.status}`);const d=await r.json();const self=getSelfPosition();const unique=(d.elements||[]).map(it=>{const lat=Number(it.lat??it.center?.lat),lng=Number(it.lon??it.center?.lon),tags=it.tags||{};const type=poiType(tags);return {id:`${it.type}:${it.id}`,lat,lng,name:poiName(tags),type,icon:poiIcon(tags),tags};}).filter(x=>Number.isFinite(x.lat)&&Number.isFinite(x.lng)).filter(x=>role==='master'||!self||L.latLng(x.lat,x.lng).distanceTo([self.lat,self.lng])<=PLAYER_VISIBILITY_METERS).filter((x,i,a)=>i===a.findIndex(y=>y.name===x.name&&Math.abs(y.lat-x.lat)<.00008&&Math.abs(y.lng-x.lng)<.00008)).slice(0,role==='master'?90:24);poiCache.set(key,{at:Date.now(),data:unique});renderPoiSet(unique);if(status)status.textContent=role==='master'?`${unique.length} locais próximos`:`${unique.length} locais dentro da sua visão`;return;}catch(e){console.warn('[AFTERLIFE][MAP][POI]',endpoint,e);}}if(status)status.textContent='Locais reais indisponíveis agora';}
