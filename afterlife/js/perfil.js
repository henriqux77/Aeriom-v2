import { aeriom, afterlifeReady } from './aeriom-client.js?v=20260914-26';
...
function renderAvatar(url,name){ const box=$('profile-avatar-large'); if(!box)return; box.replaceChildren(); if(url){ const img=document.createElement('img'); img.src=url; img.alt=''; img.onerror=()=>{box.innerHTML='<span>'+initial(name)+'</span>';}; box.appendChild(img); } else box.innerHTML='<span>'+initial(name)+'</span>'; }
...
async function load(){
  const {data,error}=await aeriom.from('profiles').select('id,display_name,avatar_path').eq('id',user.id).maybeSingle();
  if(error)throw error;
  profile=data||{};
  const {data:rc,error:rcError}=await aeriom.from('profile_recovery_contacts').select('recovery_email,recovery_phone').eq('user_id',user.id).maybeSingle();
  if(rcError)throw rcError;
  $('profile-display-name').value=profile.display_name||'';
  $('profile-recovery-email').value=rc?.recovery_email||'';
  $('profile-recovery-phone').value=rc?.recovery_phone||'';
  const name=profile.display_name||user.user_metadata?.display_name||user.email?.split('@')[0]||'Sobrevivente';
...
async function save(event){
  ...
  try{
    ...
    if(selectedFile){ const path=`${user.id}/${crypto.randomUUID()}.jpg`; const {error}=await aeriom.storage.from(BUCKET).upload(path,selectedFile,{contentType:selectedFile.type,upsert:false,cacheControl:'3600'}); if(error)throw error; avatarPath=path; }
    const {error:profileError}=await aeriom.from('profiles').upsert({id:user.id,display_name:name,avatar_path:avatarPath,updated_at:new Date().toISOString()},{onConflict:'id'}); if(profileError)throw profileError;
    const {error:metaError}=await aeriom.auth.updateUser({data:{display_name:name}}); if(metaError)throw metaError;
    if(recoveryEmail||recoveryPhone){ const {error}=await aeriom.from('profile_recovery_contacts').upsert({user_id:user.id,recovery_email:recoveryEmail||null,recovery_phone:recoveryPhone||null,updated_at:new Date().toISOString()},{onConflict:'user_id'}); if(error)throw error; }
    else { const {error}=await aeriom.from('profile_recovery_contacts').delete().eq('user_id',user.id); if(error)throw error; }
    if(profile.avatar_path&&avatarPath&&profile.avatar_path!==avatarPath) await aeriom.storage.from(BUCKET).remove([profile.avatar_path]).catch(()=>{});
    profile={...profile,display_name:name,avatar_path:avatarPath}; selectedFile=null;
    $('profile-avatar-status').textContent='Alterações salvas.'; $('profile-preview-name').textContent=name; renderAvatar(avatarPath?await signedUrl(avatarPath):null,name); message('Perfil atualizado com sucesso.','success');
