...
(() => {
  ...
  function injectStyle() {
    ...
    link.rel = 'stylesheet';
    link.href = './css/mapa-location-areas.css?v=20260916-1';
  ...
  function areaPublicCard(area) {
    ...
    card.innerHTML = `<div class="afterlife-area-card__icon">◈</div><div class="afterlife-area-card__copy"><strong>${esc(area.name)}</strong><span>${esc(area.category || 'Área')}</span><p>${esc(area.description || 'Esta área ainda não possui uma descrição pública.')}</p></div><button type="button" class="afterlife-area-card__open">ABRIR</button>`;
  ...
  function areaMasterRow(area) {
    ...
    row.innerHTML = `<div><strong>${esc(area.name)}</strong><span>${esc(area.category || 'Área')} · perigo ${esc(dangerLabel(area.danger))}${area.difficulty != null ? ` · CD ${Number(area.difficulty)}` : ''}</span></div><div class="afterlife-area-master-actions"><button type="button" data-action="edit">EDITAR</button><button type="button" data-action="delete">EXCLUIR</button></div>`;
