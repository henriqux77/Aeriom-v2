...
async function loadProfile(){
  try{
    ...
    const box=$('profileAvatar');box.replaceChildren();
...
async function selectPoint(mapInstance,latlng){
  ...
  $('regionEmpty').hidden=true;$('regionSelected').hidden=false;$('regionName').textContent='Localizando…';$('regionCoords').textContent=`${lat.toFixed(5)}°, ${lng.toFixed(5)}°`;$('coords').textContent=`${lat.toFixed(5)}, ${lng.toFixed(5)}`;$('mapState').textContent='Selecionado';
...
function clearPoint(){
  if(marker)marker.remove();marker=null;sessionStorage.removeItem('afterlife_selected_location');$('regionSelected').hidden=true;$('regionEmpty').hidden=false;$('coords').textContent='—';$('mapState').textContent='Nenhum';$('createHere').href='./campanhas.html';
}
...
function initMap(){
  ...
  $('centerSelection')?.addEventListener('click',()=>{if(marker)map.setView(marker.getLatLng(),Math.max(map.getZoom(),15),{animate:true});else map.setView([15,0],2,{animate:true});});
