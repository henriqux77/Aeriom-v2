(() => {
  ...
  function startCombatAnimationGuard() {
    ...
    const observer = new MutationObserver((records) => {
      ...
      if (records.some((record) => Array.from(record.addedNodes).some((node) => node.nodeType === 1 && node.id !== 'afterlife-combat-animation-fix'))) {
        timer = setTimeout(() => { applyCombatAnimationFix(); observer.disconnect(); }, 0);
      }
    });
  ...
  function setProfile(name, avatarUrl, email) {
    ...
    ['profileName','profileDropdownName','profileMenuName','afterlifeGlobalProfileName'].forEach((id) => {
      const el = $(id); if (el) el.textContent = display;
    });
    ['profileDropdownEmail','profileMenuEmail','afterlifeGlobalProfileEmail'].forEach((id) => {
      const el = $(id); if (el) el.textContent = email || 'Conta Afterlife';
    });
  ...
  async function loadProfile() {
    try {
      ...
      const profile = data || { id:user.id, display_name:name, avatar_path:null };
      if (data?.display_name) name = data.display_name;
      let avatarUrl = '';
      if (profile.avatar_path) {
        const signed = await sb.storage.from('avatars').createSignedUrl(profile.avatar_path,3600);
        avatarUrl = signed.data?.signedUrl || '';
      }
      setProfile(name,avatarUrl,user.email || 'Conta Afterlife');
    } catch (error) {
      console.warn('[AFTERLIFE] perfil:', error);
      setProfile('Sobrevivente','', 'Conta Afterlife');
    }
  }

  async function bootProfile() {
    if (!isAfterlifeHome()) return;
    const chip = $('profileChip') || document.querySelector('.profile-chip');
    if (!chip) return;
    const { wrap, menu } = createMenu(chip);
    ...
    if (chip.dataset.afterlifeProfileBound !== '1') {
      ...
      document.addEventListener('click',(event) => {
        if (!menu.contains(event.target) && !chip.contains(event.target)) {
          menu.hidden = true; menu.setAttribute('aria-hidden','true'); chip.setAttribute('aria-expanded','false');
        }
      },true);
      document.addEventListener('keydown',(event) => {
        if (event.key === 'Escape') { menu.hidden = true; menu.setAttribute('aria-hidden','true'); chip.setAttribute('aria-expanded','false'); }
      });
      menu.addEventListener('click',(event) => event.stopPropagation(),true);
      menu.querySelector('#afterlifeGlobalLogout')?.addEventListener('click',async (event) => {
        event.preventDefault(); event.stopPropagation();
        try { const sb = await getClient(); await sb.auth.signOut(); } finally { location.replace('../index.html'); }
      },true);
    }
    await loadProfile();
  }
