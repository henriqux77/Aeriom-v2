(() => {
  ...
  css.rel = 'stylesheet';
  css.href = './css/afterlife-profile-global.css?v=20260913-2';
  ...
  function ensureMenu() {
    ...
    menu = $('profileDropdown') || $('profileMenu') || document.querySelector('.afterlife-account-menu');
    if (menu) {
  ...
  async function loadProfile() {
    try {
      ...
      if (p.data?.avatar_path) {
        const signed = await sb.storage.from('avatars').createSignedUrl(p.data.avatar_path, 3600);
  ...
  function bind() {
    ...
    menu.querySelector('#profileLogout, #profileSignOut, #afterlifeGlobalLogout')?.addEventListener('click', async event => {
      event.preventDefault();
      event.stopPropagation();
      try {
        const mod = await import('./aeriom-client.js');
        await mod.aeriom.auth.signOut();
      } finally {
