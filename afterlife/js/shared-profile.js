...
(() => {
  ...
  function injectFix(){
    ...
    style.textContent='.profile-chip{display:flex!important;align-items:center!important;visibility:visible!important}.profile-avatar,.afterlife-global-profile-avatar,.afterlife-profile-avatar{position:relative!important;overflow:hidden!important;display:grid!important;place-items:center!important;border-radius:50%!important}.profile-avatar img,.afterlife-global-profile-avatar img,.afterlife-profile-avatar img{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;object-fit:cover!important;border-radius:50%!important;display:block!important}';
  ...
  function setAvatar(el,url,name){
    ...
    try{
      ...
      const img=document.createElement('img'); img.src=url; img.alt=''; img.loading='eager'; img.referrerPolicy='no-referrer';
  ...
  function observeAvatars(){
    const observer=new MutationObserver(()=>{
      ...
      targets.forEach(target=>{
        if(sharedState.avatarUrl){const img=target.querySelector('img');if(!img||img.src!==sharedState.avatarUrl) setAvatar(target,sharedState.avatarUrl,sharedState.name);}
      });
    });
  ...
  async function load(){
    ...
    let profile=null;
    try{
      const result=await aeriom.from('profiles').select('id,display_name,avatar_path').eq('id',user.id).maybeSingle();
      profile=result.data||null;
    }catch(error){console.warn('[AFTERLIFE] perfil:',error?.message||error);}
    ...
    let avatarUrl='';
    if(profile?.avatar_path){try{const result=await aeriom.storage.from('avatars').createSignedUrl(profile.avatar_path,3600);avatarUrl=result.data?.signedUrl||'';}catch{}}
    setProfile(name,avatarUrl,user.email||'');
  }

  function bindDropdown(){
    const chip=$('profileChip'); if(!chip||chip.dataset.sharedProfileBound==='1') return;
    const menu=$('profileDropdown')||$('profileMenu')||$('afterlifeGlobalProfileMenu'); if(!menu) return;
  ...
  async function boot(){injectFix();observeAvatars();bindDropdown();try{await load();}catch(error){console.warn('[AFTERLIFE] perfil compartilhado:',error?.message||error);}}
